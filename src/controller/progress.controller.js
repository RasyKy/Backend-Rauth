...
const saveLessonProgress = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error saving progress: " + error.message });
  }
};
...
const saveQuizAttempt = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error saving quiz attempt: " + error.message });
  }
};
...
const saveExamAttempt = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error saving exam attempt: " + error.message });
  }
};
...
const getMyQuizAttempts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching quiz attempts: " + error.message });
  }
};
...
const getMyExamAttempts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching exam attempts: " + error.message });
  }
};
...
const getAllMyQuizAttempts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching all quiz attempts: " + error.message });
  }
};
...
const getAllMyExamAttempts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching all exam attempts: " + error.message });
  }
};

const trackStudyTime = async (req, res) => {
  try {
    const { seconds } = req.body; // e.g., 60 seconds
    const userId = req.uid;

    // Create a simple date string "YYYY-MM-DD" for the Doc ID
    const today = new Date().toISOString().split("T")[0];
    const docId = `${userId}_${today}`;

    const docRef = db.collection("DailyStudyTime").doc(docId);

    await docRef.set(
      {
        userId: userId,
        date: today,
        // Atomic increment: safely adds to the existing number
