...
const createProfile = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error creating profile: " + error.message });
  }
};
...
const setRole = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Successfully set ${targetUserId} role to ${newRole}` });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error setting user role: " + error.message });
  }
};
...
const forgotPassword = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error in forgotPassword: " + error.message });
  }
};
...
const resetPassword = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error in resetPassword: " + error.message });
  }
};

const updateUser = async (req, res) => {
  try {
    const uid = req.uid; // Comes from the middleware
    const { username, role } = req.body; // Fields we want to allow updating
    ...
    // Handle Role Upgrade (e.g. Free -> Premium)
    if (role) {
      const validRoles = ["Student", "Premium", "Admin"];
      if (!validRoles.includes(role)) {
        return res
          .status(400)
          .json({ message: "Invalid role. Allowed: Student, Premium" });
      }
      updates.Role = role;

      // Critical: Update Firebase Auth Claims so the frontend knows they are Premium immediately
