...
const createExam = async (req, res) => {
    try {
        ...
        if (!module || !title) {
            return res.status(400).json({ message: 'Missing required fields: module and title.' });
        }
        ...
        res.status(201).json({ message: 'Mock exam created successfully!', id: docRef.id });
    } catch (error) {
...
const getExamsByModule = async (req, res) => {
    try {
        const snapshot = await db.collection('mockExams')
                                  .where('module', '==', req.params.moduleName)
                                  .get();
        if (snapshot.empty) return res.status(404).json({ message: 'No exams found for this module.' });
...
const getAllExams = async (req, res) => {
    try {
        const snapshot = await db.collection('mockExams').orderBy('createdAt', 'desc').get();
        const exams = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        res.status(200).json(exams);
    } catch (error) {
        res.status(500).json({ message: 'Error fetching all exams: ' + error.message });
    }
};
