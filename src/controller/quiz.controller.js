...
const createQuiz = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Quiz created successfully!', id: docRef.id });
    } catch (error) {
...
const getQuizzesByModule = async (req, res) => {
    try {
        const snapshot = await db.collection('quizzes')
                                  ...
                                  .get();
        if (snapshot.empty) return res.status(404).json({ message: 'No quizzes found for this module.' });
...
const getAllQuizzes = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error fetching all quizzes: ' + error.message });
    }
};
